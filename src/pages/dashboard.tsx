...
import { getLabResults } from 'apiSdk/lab-results';
import { LabResultInterface } from 'interfaces/lab-result';

import { LabResultListPage as ShowCaseList } from 'pages/lab-results';
...
function DashboardPage() {
  ...
  const prescriptionStat = useSWR<PaginatedInterface<PrescriptionInterface>>(
    () => `/prescriptions`,
    () => getPrescriptions({ limit: 1, offset: 0 }),
  );

  const labResultStat = useSWR<PaginatedInterface<LabResultInterface>>(
    () => `/lab-results`,
    () => getLabResults({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Prescriptions',
      result: prescriptionStat?.data?.totalCount,
      isLoading: prescriptionStat.isLoading,
    },
    {
      title: 'Lab Results',
      result: labResultStat?.data?.totalCount,
      isLoading: labResultStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`lab-results`} passHref legacyBehavior>
                  <Button
