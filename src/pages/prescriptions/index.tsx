...
type ColumnType = ColumnDef<PrescriptionInterface, unknown>;
...
export function PrescriptionListPage(props: PrescriptionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PrescriptionInterface>>(
    () => `/prescriptions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PrescriptionInterface) => {
    if (hasAccess('prescription', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/prescriptions/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/prescriptions/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="prescription" />
        </Flex>
